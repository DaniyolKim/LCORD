  ...
  <div>
    <form @submit.prevent="onSubmit">
      ...
      <div>
        ...
        <div>
          <select v-model="leagueData.tierMin"><option v-for="tier in $defs.tierList" :value="tier.val">{{tier.name}}</option></select> ~
          <select v-model="leagueData.tierMax"><option v-for="tier in $defs.tierList" :value="tier.val">{{tier.name}}</option></select>
        </div>
      </div>
      <div>
        <label>진행 방식 :
